...
- **React Hooks Layer (Partial)** — Implemented tRPC client infrastructure (`src/lib/trpc-client.ts`) and React Query hooks for `useOffline`, `useEvent`, `usePotluck`, and `useHousehold`. Created `TRPCProvider` component wrapping the app with tRPC and React Query context. Added `/api/trpc/[trpc]` API route for tRPC requests. Hooks wrap tRPC procedures for event queries, RSVP status, potluck slots/signups, household data, and dependent management. Migration of existing components (`RSVPForm`, `PotluckSignupForm`, `ProfileClient`, `PhotoReactionButton`) to use hooks is pending. Tests at `src/hooks/__tests__/hooks.test.ts`.
