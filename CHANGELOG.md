...
- **Registration fee backfill coverage** — `src/lib/__tests__/registration-fee-backfill.test.ts` (13 tests) covers dry-run no-op, apply zeros non-settled + audits each, settled-row protection (PAID / REFUNDED / FORFEITED / CANCELLED left alone), idempotency on a re-run, per-row error isolation, top-level scan failure, and the formatter output. `tests/integration/registration-fee-backfill-script.test.ts` (9 tests) verifies the script wiring: thin CLI wrapper, `--apply` flag handling, exit non-zero on errors, package script registered, lib exports, audit action name, `$transaction` wrapping, and explicit settled-status guard.
